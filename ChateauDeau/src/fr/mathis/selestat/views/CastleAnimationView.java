...
public class CastleAnimationView extends View {
	private static final String LOG_TAG = "CastleAnimationView";
	...
	public CastleAnimationView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public CastleAnimationView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);

		mPaint.setStyle(Paint.Style.STROKE);

		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CastleAnimationView, defStyle, 0);
		try {
			if (a != null) {
				mPaint.setStrokeWidth(Tools.convertDpToPixel(a.getFloat(R.styleable.CastleAnimationView_strokeWidth, 1.0f)));
				mPaint.setColor(a.getColor(R.styleable.CastleAnimationView_strokeColor, 0xff000000));
				mPhase = a.getFloat(R.styleable.CastleAnimationView_phase, 0.0f);
				mDuration = a.getInt(R.styleable.CastleAnimationView_duration, 4000);
				mFadeFactor = a.getFloat(R.styleable.CastleAnimationView_fadeFactor, 10.0f);
			}
		} finally {
			if (a != null)
				a.recycle();
	...
	public void reveal(View scroller, int parentBottom, int repeat) {
		if (mSvgAnimator == null) {
			mSvgAnimator = ObjectAnimator.ofFloat(this, "phase", mPhase, 0.0f);
			mSvgAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
			mSvgAnimator.setDuration(mDuration);
			if (repeat > 0) {
				mSvgAnimator.setRepeatMode(Animation.REVERSE);
				mSvgAnimator.setRepeatCount(repeat);
			}
			mSvgAnimator.start();
		}
